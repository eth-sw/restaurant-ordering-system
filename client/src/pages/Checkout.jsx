...
const Checkout = () => {
    ...
    const handleVerifyAddress = async (e) => {
        ...
        try {
            ...
            setMessage("Error validating address, try again.");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', padding: '20px' }}>
                ...
                {basketItems.map(item => (
                    <div key={item._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #eee', padding: '15px 0' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <button onClick={() => decreaseQuantity(item._id)} style={{ width: '30px', height: '30px', borderRadius: '50%', border: '1px solid #ccc', cursor: 'pointer' }}>-</button>
                            <span style={{ fontWeight: 'bold' }}>{item.qty}</span>
                            <button onClick={() => addToBasket(item)} style={{ width: '30px', height: '30px', borderRadius: '50%', border: '1px solid #ccc', cursor: 'pointer' }}>+</button>
                            <button onClick={() => removeFromBasket(item._id)} style={{ marginLeft: '10px', color: '#d32f2f', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Remove</button>
                        </div>
            ...
            <div style={{ background: '#fff', border: '1px solid #ddd', borderRadius: '8px', padding: '20px' }}>
                ...
                <form onSubmit={handleVerifyAddress} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    <input type="tel" name="phone" placeholder="Phone (+44...)" value={customerInfo.phone} onChange={onInfoChange} required style={{ padding: '10px' }} disabled={addressVerified} />
                    <input type="text" name="address" placeholder="Full Delivery Address" value={customerInfo.address} onChange={onInfoChange} required style={{ padding: '10px' }} disabled={addressVerified} />

                    {!addressVerified && (
                        <button type="submit" disabled={loading} style={{ padding: '12px', background: '#1976d2', color: 'white', border: 'none', cursor: 'pointer' }}>
                ...
                {addressVerified && !clientSecret && (
                    <button onClick={initPayment} disabled={loading} style={{ width: '100%', marginTop: '20px', padding: '15px', background: '#2e7d32', color: 'white', border: 'none', cursor: 'pointer' }}>
                        Proceed to payment
                    </button>
                )}
                {clientSecret && (
